...
class Booking(models.Model):
    ...
    earnings_amount = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Earnings from booking"
    )
    ...
    def check_conflicts(self):
        ...
        same_day_conflicts = (
            ...
            .exclude(property=self.property)  # Exclude same property
...
class Task(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="cleaning_tasks", null=True, blank=True
    )
    assigned_to = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, related_name="assigned_tasks", null=True, blank=True
    )
    modified_at = models.DateTimeField(auto_now=True)
    modified_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, related_name="modified_tasks", null=True, blank=True
    )
    due_date = models.DateTimeField(null=True, blank=True, help_text="Optional deadline for task (stored in UTC)")
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            if old.status != self.status:
                changes.append(
                    f"{timezone.now().isoformat()}: {user} changed status " f"from '{old.status}' to '{self.status}'"
                )
            ...
            if old.assigned_to_id != self.assigned_to_id:
                ...
                changes.append(f"{timezone.now().isoformat()}: {user} changed assignee " f"from '{old_name}' to '{new_name}'")

            # Title change
            if old.title != self.title:
                changes.append(f"{timezone.now().isoformat()}: {user} changed title " f"from '{old.title}' to '{self.title}'")
            ...
            if old.due_date != self.due_date:
                ...
                changes.append(f"{timezone.now().isoformat()}: {user} changed due_date " f"from '{old_dt}' to '{new_dt}'")
...
class Profile(models.Model):
    ...
    phone_number = models.CharField(max_length=20, blank=True, null=True, help_text="User's phone number (optional)")
    ...
    def get_departments(self):
        ...
        return list(self.user.groups.filter(name__in=DepartmentGroups.get_all_departments()).values_list("name", flat=True))
    ...
    def get_all_permissions(self):
        ...
        for role_perm in RolePermission.objects.filter(role=self.role, permission__is_active=True):
            permissions[role_perm.permission.name] = role_perm.granted

        # Apply user overrides (these take precedence)
        for override in self.user.permission_overrides.filter(permission__is_active=True):
    ...
    def get_delegatable_permissions(self):
        ...
        return RolePermission.objects.filter(
            role=self.role, permission__is_active=True, granted=True, can_delegate=True
...
class UserPermissionOverride(models.Model):
    ...
    granted_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="permissions_granted"
    )
        ...
        if self.pk:
            try:
                ...
                if old.read != self.read:
                    ...
                    changes.append(f"{timezone.now().isoformat()}: {user} {status}")

                # Verb change
                if old.verb != self.verb:
                    changes.append(f"{timezone.now().isoformat()}: {user} changed verb " f"from '{old.verb}' to '{self.verb}'")
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            try:
                ...
                if old.name != self.name:
                    changes.append(
                        f"{timezone.now().isoformat()}: {user} changed template name " f"from '{old.name}' to '{self.name}'"
                    )
                ...
                if old.is_active != self.is_active:
                    ...
                    changes.append(f"{timezone.now().isoformat()}: {user} {status} template")
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            try:
                ...
                # Name change
                if old.name != self.name:
                    changes.append(f"{timezone.now().isoformat()}: {user} changed name " f"from '{old.name}' to '{self.name}'")
                ...
                if old.is_active != self.is_active:
                    status = "activated" if self.is_active else "deactivated"
                    changes.append(f"{timezone.now().isoformat()}: {user} {status} item")
        ...
        if self.pk:
            try:
                ...
                if old.par_level != self.par_level:
                    changes.append(
                        f"{timezone.now().isoformat()}: {user} changed par level " f"from {old.par_level} to {self.par_level}"
                    )

                # Max level change
                if old.max_level != self.max_level:
                    changes.append(
                        f"{timezone.now().isoformat()}: {user} changed max level " f"from {old.max_level} to {self.max_level}"
                    )
...
class InventoryTransaction(models.Model):
    ...
    def __str__(self):
        ...
        return (
            f"{self.get_transaction_type_display()}: {sign}{self.quantity} {self.property_inventory.item.get_unit_display()}"
        )
        ...
        if self.pk:
            try:
                ...
                if old.quantity != self.quantity:
                    changes.append(
                        f"{timezone.now().isoformat()}: {user} changed quantity " f"from {old.quantity} to {self.quantity}"
                    )

                # Notes change
                if old.notes != self.notes:
                    changes.append(
                        f"{timezone.now().isoformat()}: {user} changed notes " f"from '{old.notes}' to '{self.notes}'"
                    )
...
class LostFoundItem(models.Model):
    ...
    task = models.ForeignKey(
        "Task", on_delete=models.SET_NULL, null=True, blank=True, help_text="Task during which item was found"
    )
    booking = models.ForeignKey(
        "Booking", on_delete=models.SET_NULL, null=True, blank=True, help_text="Associated booking if known"
    )
...
class ScheduleTemplate(models.Model):
    ...
    property_ref = models.ForeignKey(
        "Property", on_delete=models.CASCADE, related_name="schedule_templates", null=True, blank=True
    )
    ...
    def should_generate_task(self, check_date=None):
        ...
        return check_date >= create_date and (not self.last_generated or self.last_generated.date() < create_date)
        ...
        if self.pk:
            try:
                ...
                if old.schedule_id != self.schedule_id:
                    ...
                    changes.append(
                        f"{timezone.now().isoformat()}: {user} changed schedule " f"from '{old_schedule}' to '{new_schedule}'"
                    )
...
class BookingImportLog(models.Model):
    ...
    template = models.ForeignKey(
        BookingImportTemplate, on_delete=models.CASCADE, related_name="import_logs", null=True, blank=True
    )
