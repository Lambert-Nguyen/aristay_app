...
class ImportFileCleanupService:
    ...
    def cleanup_old_files(days_to_keep: int = 30, dry_run: bool = False) -> Dict[str, Any]:
        ...
        old_logs = BookingImportLog.objects.filter(imported_at__lt=cutoff_date, import_file__isnull=False).exclude(
            import_file=""
        )
    ...
    def get_storage_stats() -> Dict[str, Any]:
        ...
        import_logs = BookingImportLog.objects.filter(import_file__isnull=False).exclude(import_file="")
    ...
    def suggest_cleanup(target_size_mb: int = 100) -> Dict[str, Any]:
        ...
        for days in [7, 14, 30, 60, 90]:
            cleanup_result = ImportFileCleanupService.cleanup_old_files(days_to_keep=days, dry_run=True)
