...
from api.models import Booking, BookingImportLog, BookingImportTemplate, Property, Task
...
class ExcelImportService:
    ...
    def import_excel_file(self, excel_file, sheet_name: str = "Cleaning schedule") -> Dict:
        ...
        try:
            ...
            if df is None or df.empty:
                return {"success": False, "error": f'No data found in sheet "{sheet_name}" or file is empty'}
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    logger.debug(
                        f"Processing row {index + 2}: {row.get('Guest name', 'No guest name')} - {row.get('Properties', 'No property')}"
                    )
            ...
            return {
                ...
                "new_properties_list": [
                    w.replace("✨ Created new property: ", "")
                    for w in self.warnings
                    if w.startswith("✨ Created new property:")
                ],
            }
            ...
            try:
                ...
                if first_property:
                    self.template, created = BookingImportTemplate.objects.get_or_create(
                        ...
                        defaults={"import_type": "csv", "auto_create_tasks": True, "created_by": self.user},
                    )
                else:
    ...
    def _extract_booking_data(self, row: pd.Series, row_number: int) -> Optional[Dict]:
        ...
        try:
            ...
            if data.get("external_code"):
                ...
                if counter > 1:
                    logger.warning(
                        f"Row {row_number}: Duplicate external code '{original_code}' - generated unique code: '{data['external_code']}'"
                    )
    ...
    def _find_or_create_property(self, property_label: str) -> Optional[Property]:
        ...
        if self.user.is_superuser:
            ...
            try:
                ...
                property_obj = Property.objects.create(name=property_label, created_by=self.user, modified_by=self.user)
    ...
    def _find_existing_booking(self, booking_data: Dict, property_obj: Property) -> Optional[Booking]:
        ...
        try:
            ...
            return Booking.objects.get(external_code=booking_data["external_code"])
    ...
    def _check_booking_conflicts(
        self, booking_data: Dict, property_obj: Property, existing_booking: Optional[Booking] = None
    ) -> List[str]:
        ...
        overlapping_bookings = Booking.objects.filter(
            property=property_obj, check_in_date__lt=end_date, check_out_date__gt=start_date
        )
        ...
        try:
            ...
            if associated_task and "external_status" in booking_data:
                ...
                if associated_task.status != new_task_status:
                    ...
                    logger.info(
                        f"Updated task {associated_task.id} status from '{old_status}' to '{new_task_status}' for booking {booking.external_code}"
                    )
    ...
    def _update_import_log(self):
        ...
        if self.import_log:
            ...
            logger.info(
                f"Import completed: {self.success_count}/{self.total_rows} successful, {len(self.errors)} errors, {len(self.warnings)} warnings"
            )
