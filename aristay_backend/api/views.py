...
from .models import Booking, Device, Notification, Property, PropertyOwnership, Task, TaskImage
from .permissions import (
    ...
    DynamicTaskPermissions,
    DynamicUserPermissions,
    HasCustomPermission,
    IsManagerOrOwner,
    IsOwner,
    IsOwnerOrAssignedOrReadOnly,
)
...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def count_by_status(self, request):
        ...
        counts = qs.values("status").annotate(count=Count("id")).order_by()  # avoid implicit GROUP BY ordering issues

        by_status = {entry["status"]: entry["count"] for entry in counts}

        now = timezone.now()
        overdue = qs.filter(due_date__isnull=False, due_date__lt=now).exclude(status__in=["completed", "canceled"]).count()
...
def portal_property_list(request):
    ...
    for p in props:
        ...
        current = sum(
            1 for b in bookings if b.check_in_date and b.check_out_date and b.check_in_date <= now < b.check_out_date
        )
...
def portal_task_detail(request, task_id):
    ...
    accessible_properties = _accessible_properties_for(request.user)
    if not (request.user.is_staff or task.assigned_to == request.user or task.property in accessible_properties):
...
class DeviceRegisterView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        device, created = Device.objects.update_or_create(user=request.user, defaults={"token": token})

        return Response({"status": "registered", "token": device.token, "created": created})
...
class NotificationListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Notification.objects.filter(recipient=self.request.user).order_by("-timestamp", "-id")  # newest first, stable
...
def mark_all_notifications_read(request):
    count = Notification.objects.filter(recipient=request.user, read=False).update(read=True, read_at=timezone.now())
...
def manager_charts_dashboard(request):
    ...
    tasks_by_property = (
        Task.objects.select_related("property")
    ...
    tasks_by_type = Task.objects.values("task_type").annotate(count=Count("id")).order_by("task_type")

    # Get overdue tasks
    now = timezone.now()
    overdue_count = (
        Task.objects.filter(due_date__isnull=False, due_date__lt=now).exclude(status__in=["completed", "canceled"]).count()
    )
    ...
    daily_completions = (
        Task.objects.filter(status="completed", modified_at__gte=thirty_days_ago)
...
def admin_charts_dashboard(request):
    ...
    if not request.user.is_superuser:
        if not (
            hasattr(request.user, "profile") and request.user.profile and request.user.profile.has_permission("view_reports")
        ):
            from django.contrib.auth.views import redirect_to_login
...
def system_metrics_dashboard(request):
    ...
    if not request.user.is_superuser:
        if not (
            ...
            and request.user.profile.has_permission("system_metrics_access")
        ):
            from django.core.exceptions import PermissionDenied
...
def excel_import_view(request):
        ...
        try:
            ...
            if first_property:
                template, created = BookingImportTemplate.objects.get_or_create(
                    ...
                    defaults={"import_type": "csv", "auto_create_tasks": True, "created_by": request.user},
                )
                if created:
            ...
            if result["success"]:
                logger.info(
                    f"Excel import successful for user {request.user.username}: {result['successful_imports']} bookings, {result['errors_count']} errors, {result['warnings_count']} warnings"
                )
                ...
                if result["errors"]:
                    ...
                    if len(result["errors"]) > 5:
                        messages.warning(
                            request, f"... and {len(result['errors']) - 5} more errors. Check the import log for details."
                        )
                ...
                if result.get("warnings_count", 0) > 0:
                    ...
                    if warnings_list:
                        ...
                        if len(warnings_list) > 5:
                            messages.warning(
                                request,
                                f"⚠️ ... and {len(warnings_list) - 5} more warnings. Check import log for full details.",
                            )
                    else:
                        messages.warning(
                            request,
                            f"Import completed with {result['warnings_count']} warnings. Check the import log for details.",
                        )
                ...
                if result.get("new_properties_created", 0) > 0:
                    ...
                    if new_properties_list:
                        ...
                        messages.success(
                            request, f"🎉 Created {result['new_properties_created']} new properties during import."
                        )
    ...
    if hasattr(request.user, "profile") and request.user.profile.role in ["manager", "superuser"]:
        ...
        recent_imports = BookingImportLog.objects.filter(imported_by=request.user).order_by("-imported_at")[:10]
...
def excel_import_api(request):
    ...
    try:
        ...
        logger.info(
            f"Excel import API result for user {request.user.username}: success={result.get('success')}, bookings={result.get('successful_imports', 0)}"
        )

        # If properties need approval, return special response
        if result.get("requires_property_approval"):
            result["message"] = (
                f"Import requires admin approval. Found {len(result.get('new_properties', []))} new properties."
            )
            logger.info(
                f"Excel import requires property approval for user {request.user.username}: {len(result.get('new_properties', []))} new properties"
            )
        ...
        if result.get("new_properties_created", 0) > 0:
            ...
            if new_properties_list:
                result["success_message"] = (
                    f"🎉 Successfully created {result['new_properties_created']} new properties: {', '.join(new_properties_list[:5])}{'...' if len(new_properties_list) > 5 else ''}"
                )
            else:
                result["success_message"] = f"🎉 Created {result['new_properties_created']} new properties during import."
            logger.info(
                f"Excel import created {result['new_properties_created']} new properties for user {request.user.username}"
            )
...
def property_approval_create(request):
    ...
    try:
        ...
        for property_name in approved_properties:
            try:
                Property.objects.create(name=property_name.strip(), created_by=request.user, modified_by=request.user)
                created_count += 1
...
from api.services.enhanced_excel_import_service import ConflictResolutionService, EnhancedExcelImportService
...
def quick_resolve_conflict(request, import_session_id, conflict_index):
    ...
    try:
        ...
        if action == "auto_update":
            ...
            resolution = {"conflict_index": int(conflict_index), "action": "create_new", "apply_changes": []}
        else:  # skip
            resolution = {"conflict_index": int(conflict_index), "action": "skip", "apply_changes": []}

        # Resolve using service
        resolution_service = ConflictResolutionService(request.user)
        results = resolution_service.resolve_conflicts(import_session_id, [resolution])

        return JsonResponse({"success": True, "action": action, "results": results})
...
def file_cleanup_api(request):
    ...
    if request.method == "GET":
        ...
        try:
            if action == "stats":
                stats = ImportFileCleanupService.get_storage_stats()
                return JsonResponse({"success": True, "action": "stats", "stats": stats})
                ...
                return JsonResponse({"success": True, "action": "suggest", "suggestion": suggestion})
...
def available_permissions(request):
    ...
    try:
        ...
        if profile.role == UserRole.SUPERUSER:
            ...
            permissions = CustomPermission.objects.filter(name__in=delegatable, is_active=True)
        ...
        return Response({"permissions": permissions_data})
    except Exception as e:
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def manageable_users(request):
    ...
    try:
        ...
        users = User.objects.filter(profile__role__in=manageable_roles, is_active=True).select_related("profile")
...
def grant_permission(request):
    ...
    try:
        ...
        if not profile.can_delegate_permission(permission_name):
            return Response(
                {"error": "You do not have permission to delegate this permission"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        if not hasattr(target_user, "profile"):
            return Response({"error": "Target user has no profile"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if target_role not in manageable_roles:
            return Response({"error": "You cannot manage permissions for this user"}, status=status.HTTP_403_FORBIDDEN)

        # Get permission object
        try:
            permission_obj = CustomPermission.objects.get(name=permission_name, is_active=True)
        except CustomPermission.DoesNotExist:
            return Response({"error": "Permission not found or inactive"}, status=status.HTTP_404_NOT_FOUND)
        ...
        override, created = UserPermissionOverride.objects.update_or_create(
            ...
            defaults={"granted": True, "granted_by": request.user, "reason": reason, "expires_at": expires_datetime},
        )
...
def revoke_permission(request):
    ...
    try:
        ...
        if not profile.can_delegate_permission(permission_name):
            return Response(
                {"error": "You do not have permission to manage this permission"}, status=status.HTTP_403_FORBIDDEN
            )
...
def remove_permission_override(request):
    ...
    try:
        ...
        try:
            override = UserPermissionOverride.objects.get(user=target_user, permission=permission_obj)
            override.delete()
            return Response(
                {
                    "success": True,
            ...
            return Response({"success": True, "message": "No override existed for this permission"})
...
def permission_management_view(request):
    ...
    if not request.user.is_superuser:
        ...
        if not (
            ...
            and request.user.profile.get_delegatable_permissions()
        ):
            from django.contrib.auth.views import redirect_to_login
