...
class IsOwnerOrAssignedOrReadOnly(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        user = request.user
        return user.is_staff or task.created_by == user or task.assigned_to == user
...
class CanManageFiles(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if hasattr(request.user, "profile") and request.user.profile:
            return request.user.profile.has_permission("excel_import_access") or request.user.profile.has_permission(
                "file_upload_access"
            )
