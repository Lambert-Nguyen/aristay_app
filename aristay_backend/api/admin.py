...
class TaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("History", {"fields": ("history",), "classes": ("collapse",)}),
    )
...
class PropertyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "address", "created_by", "modified_by")}),
        ("History", {"fields": ("history", "created_at", "modified_at"), "classes": ("collapse",)}),
    )
...
class BookingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("property", "guest_name", "guest_contact", "status", "check_in_date", "check_out_date")}),
        (
            "Excel Import Data",
            {
                "fields": (
        ...
        (
            "Timing & Notes",
            {"fields": ("check_in_time", "check_out_time", "same_day_note", "same_day_flag"), "classes": ("collapse",)},
        ),
        (
            "Tracking",
            {"fields": ("created_at_dual", "modified_at_dual", "raw_row", "last_import_update"), "classes": ("collapse",)},
        ),
        ("History", {"fields": ("history",), "classes": ("collapse",)}),
    )
...
class AriStayUserAdmin(DjangoUserAdmin):
    ...
    def user_change_password(self, request, id, form_url=""):
        ...
        if not request.user.is_superuser:
            messages.error(
                request,
                "You don't have permission to change user passwords. Use the 'Send Password Reset Email' button instead.",
            )
...
class ChecklistTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "task_type", "description", "is_active", "created_by")}),
        ("History", {"fields": ("history", "created_at"), "classes": ("collapse",)}),
    )
...
class InventoryItemAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "category", "unit", "description", "brand", "sku", "estimated_cost", "is_active")}),
        ("History", {"fields": ("history", "created_at"), "classes": ("collapse",)}),
    )
...
class PropertyInventoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("History", {"fields": ("history", "last_updated"), "classes": ("collapse",)}),
    )

    def stock_status(self, obj):
        status = obj.stock_status
        colors = {"out_of_stock": "red", "low_stock": "orange", "normal": "green", "overstocked": "blue"}
        from django.utils.html import format_html

        return format_html('<span style="color: {}">{}</span>', colors.get(status, "black"), status.replace("_", " ").title())
...
class LostFoundItemAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("title", "description", "category", "estimated_value", "property_ref", "task", "booking")}),
        ("Location & Status", {"fields": ("found_location", "status", "storage_location")}),
        ("Found Info", {"fields": ("found_date", "found_by"), "classes": ("collapse",)}),
        ("Claimed Info", {"fields": ("claimed_date", "claimed_by"), "classes": ("collapse",)}),
        ("Disposal Info", {"fields": ("disposal_date", "disposal_method"), "classes": ("collapse",)}),
...
class ScheduleTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "task_type", "property_ref", "is_active")}),
        (
            "Task Template",
            {"fields": ("task_title_template", "task_description_template", "default_assignee", "checklist_template")},
        ),
        ("Schedule", {"fields": ("frequency", "interval", "weekday", "day_of_month", "start_date", "end_date")}),
        ("Timing", {"fields": ("time_of_day", "advance_days")}),
        ("System Info", {"fields": ("created_at", "last_generated"), "classes": ("collapse",)}),
    )
...
class GeneratedTaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("schedule", "task", "generated_for_date")}),
        ("History", {"fields": ("history", "generated_at"), "classes": ("collapse",)}),
    )
...
class BookingImportTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "property_ref", "import_type", "is_active")}),
        (
            "CSV Field Mapping",
            {
...
class RolePermissionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("role", "permission", "granted", "can_delegate")}),
        ("System Info", {"fields": ("created_by", "created_at"), "classes": ("collapse",)}),
    )
...
class UserPermissionOverrideAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("user", "permission", "granted", "reason")}),
        ("Expiration", {"fields": ("expires_at",), "description": "Leave blank for permanent override"}),
        ("System Info", {"fields": ("granted_by", "created_at", "is_expired_display"), "classes": ("collapse",)}),
    )
...
class ProfileAdminWithPermissions(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("user", "role", "timezone")}),
        ("Contact Info", {"fields": ("phone_number", "address"), "classes": ("collapse",)}),
        ("Notifications", {"fields": ("digest_opt_out",)}),
        (
            "Permissions Summary",
            {
