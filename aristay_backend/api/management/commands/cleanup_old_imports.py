...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if keep:
            ...
            logs_to_delete = BookingImportLog.objects.filter(imported_at__lt=cutoff_date)
        ...
        for log in logs_to_delete:
            if log.import_file:
                ...
                if os.path.exists(file_path):
                    ...
                    files_to_delete.append({"log": log, "path": file_path, "size": file_size})
                    total_size += file_size

        # Display summary
        if not files_to_delete:
            self.stdout.write(self.style.WARNING(f"No files found matching criteria: {criteria}"))
            return
        ...
        for file_info in files_to_delete:
            ...
            self.stdout.write(
                f"  {log.imported_at.strftime('%Y-%m-%d %H:%M')} | " f"{size_str:>8} | " f"{log.import_file.name}"
            )
