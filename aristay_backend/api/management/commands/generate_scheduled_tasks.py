...
from api.models import ChecklistResponse, GeneratedTask, Property, ScheduleTemplate, Task, TaskChecklist
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for schedule in schedules:
            if schedule.should_generate_task(check_date):
                try:
                    if dry_run:
                        self.stdout.write(f'Would generate: {schedule.name} for {schedule.property_ref or "All Properties"}')
                        generated_count += 1
                    else:
                        task = self.generate_task_from_schedule(schedule, check_date)
                        if task:
                            self.stdout.write(self.style.SUCCESS(f"Generated: {task.title}"))
                            generated_count += 1
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f"Error generating task for {schedule.name}: {str(e)}"))
    ...
    def generate_task_from_schedule(self, schedule, check_date):
        ...
        if schedule.checklist_template:
            task_checklist = TaskChecklist.objects.create(task=task, template=schedule.checklist_template)
            ...
            for item in schedule.checklist_template.items.all():
                ChecklistResponse.objects.create(
                    checklist=task_checklist, item=item, is_completed=False  # Start with all items uncompleted
                )

        # Record that this task was generated
        GeneratedTask.objects.create(schedule=schedule, task=task, generated_for_date=due_date)
