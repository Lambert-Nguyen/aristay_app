    ...
    def handle(self, *args, **options):
        ...
        for permission_name, description in CustomPermission.PERMISSION_CHOICES:
            permission, created = CustomPermission.objects.get_or_create(
                name=permission_name, defaults={"description": description}
            )
            if created:
            ...
            for permission_name in config["permissions"]:
                try:
                    ...
                    role_perm, created = RolePermission.objects.get_or_create(
                        role=role, permission=permission, defaults={"granted": True, "can_delegate": can_delegate}
                    )
                    ...
                    self.stdout.write(self.style.WARNING(f"Permission {permission_name} not found"))
