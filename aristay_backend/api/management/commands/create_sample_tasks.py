...
class Command(BaseCommand):
        ...
        for prop in properties:
            ...
            if bookings:
                ...
                for i in range(random.randint(1, 2)):
                    ...
                    task_checklist = TaskChecklist.objects.create(task=task, template=cleaning_template)
    ...
    def create_maintenance_tasks(self, properties, admin_user):
        ...
        for prop in properties:
            ...
            # Create checklist for this task
            task_checklist = TaskChecklist.objects.create(task=task, template=maintenance_template)

            # Create responses for all checklist items
            for item in maintenance_template.items.all():
                ChecklistResponse.objects.create(
                    checklist=task_checklist, item=item, is_completed=random.choice([True, False])
                )
    ...
    def create_laundry_tasks(self, properties, admin_user):
        ...
        for prop in properties:
            ...
            if bookings:
                ...
                # Create checklist for this task
                task_checklist = TaskChecklist.objects.create(task=task, template=laundry_template)
