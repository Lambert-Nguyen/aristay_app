...
class Command(BaseCommand):
    ...
    def create_checklist_templates(self, admin_user):
        ...
        if created:
            cleaning_items = [
                ...
                # Bedroom items
                {
                    "title": "Strip and remake bed with fresh linens",
                    "room_type": "bedroom",
                    "item_type": "blocking",
                    "order": 1,
                },
            ...
            for item_data in cleaning_items:
                ChecklistItem.objects.create(template=cleaning_template, **item_data)
        ...
        if created:
            ...
            for item_data in maintenance_items:
                ChecklistItem.objects.create(template=maintenance_template, **item_data)
        ...
        if created:
            ...
            for item_data in laundry_items:
                ChecklistItem.objects.create(template=laundry_template, **item_data)
    ...
    def create_inventory_system(self, admin_user):
        ...
        for cat_data in categories_data:
            category, created = InventoryCategory.objects.get_or_create(name=cat_data["name"], defaults=cat_data)
        ...
        for item_data in items_data:
            ...
            item, created = InventoryItem.objects.get_or_create(name=item_data["name"], category=category, defaults=item_data)
