...
class Property(models.Model):
    ...
    def current_booking(self):
        ...
        now = timezone.now()
        return self.bookings.filter(check_in_date__lte=now, check_out_date__gte=now, status="active").first()

    @property
    def upcoming_bookings(self):
        """Get upcoming bookings"""
        now = timezone.now()
        return self.bookings.filter(check_in_date__gt=now, status="upcoming").order_by("check_in_date")

    @property
    def past_bookings(self):
        """Get completed bookings"""
        now = timezone.now()
        return self.bookings.filter(check_out_date__lt=now, status="completed").order_by("-check_out_date")
...
class Profile(models.Model):
    ...
    timezone = models.CharField(max_length=32, choices=[(tz, tz) for tz in sorted(available_timezones())], default="UTC")
    digest_opt_out = models.BooleanField(default=False)

    # Enhanced role system
    role = models.CharField(
        max_length=20, choices=UserRole.choices, default=UserRole.CLEANING_STAFF, help_text="Primary role for this user"
    )
