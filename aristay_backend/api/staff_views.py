...
def cleaning_dashboard(request):
    ...
    assigned_tasks = (
        Task.objects.filter(assigned_to=request.user, task_type="cleaning", status__in=["pending", "in_progress"])
        .select_related("property", "booking")
        .prefetch_related("checklist__responses")
    )
    ...
    upcoming_tasks = assigned_tasks.filter(due_date__date__gt=today, due_date__date__lte=today + timezone.timedelta(days=7))
    ...
    for task in assigned_tasks:
        ...
        tasks_with_progress.append({"task": task, "progress": progress, "checklist": getattr(task, "checklist", None)})
...
def task_detail(request, task_id):
    ...
    task = get_object_or_404(Task.objects.select_related("property", "booking", "assigned_to"), id=task_id)
...
def update_checklist_response(request, response_id):
    ...
    try:
        ...
        return JsonResponse({"success": True, "progress": progress, "is_completed": response.is_completed})
...
def my_tasks(request):
    ...
    tasks = Task.objects.filter(assigned_to=request.user).select_related("property", "booking").order_by("-due_date")
    ...
    if search:
        tasks = tasks.filter(
            Q(title__icontains=search) | Q(description__icontains=search) | Q(property__name__icontains=search)
        )
...
def log_inventory_transaction(request):
    ...
    try:
        ...
        inventory_item = get_object_or_404(PropertyInventory, id=data["inventory_id"])
        ...
        return JsonResponse(
            {"success": True, "new_stock": float(inventory_item.current_stock), "status": inventory_item.stock_status}
        )
...
def lost_found_list(request):
    ...
    if request.user.is_staff:
        ...
        property_ids = Task.objects.filter(assigned_to=request.user).values_list("property", flat=True).distinct()
...
def upload_checklist_photo(request):
    ...
    try:
        ...
        # Create photo record
        checklist_photo = ChecklistPhoto.objects.create(response=response, image=photo, uploaded_by=request.user)

        return JsonResponse({"success": True, "photo_id": checklist_photo.id, "photo_url": checklist_photo.image.url})
...
def set_task_status(request, task_id):
    ...
    try:
        ...
        logger.info(
            f"Task {task_id} status updated successfully: {old_status} -> {new_status} by user {request.user.username}"
        )
...
def task_counts_api(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "counts": task_counts})
    except Exception as e:
